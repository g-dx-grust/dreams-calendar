...
本書は、社内のスケジュール共有システム「**G-DX For スケジュール**」（サイボウズ風の業務カレンダー）について、現時点で実装済みの画面と機能を、操作手順とともにご説明する資料です。
...
予定を **日 / 週 / 月** の 3 つの表示で確認できます。画面上部のタブで表示を切り替え、「<」「>」「今日」と日付欄で期間を移動します。担当者のチップで、表示する社員を絞り込めます。各予定をクリックすると詳細・編集画面に移動します。

### 3.1 日表示

![カレンダー 日表示](screenshots/01-calendar-day.png)

社員を縦軸、時間を横軸にしたタイムライン表示です。予定は種別の色で表示され、複数の担当者がいる予定には人数（例：+2）が付きます。画面下部の凡例で色と種別の対応を確認できます。さらに下部には当日の**日報**欄があります。
...
カレンダーの日表示の下部に、その日の日報欄があります。担当者が当日の業務内容を記録・提出でき、提出時には Lark への通知連携にも対応しています（通知の有効化は Lark 連携設定後）。

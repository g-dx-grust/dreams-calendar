...
- オンライン予定種別と暫定会議URL生成。
- 固定`LARK_CALENDAR_ID`を使ったLarkカレンダー同期の土台。
...
固定`LARK_CALENDAR_ID`同期の土台を、事故が起きにくい運用にする。

実装すること:

1. `/api/lark/calendar/sync`は本番で`LARK_SYNC_SECRET`必須にする。
2. `LARK_SYNC_SECRET`未設定時に全通しする挙動は、開発環境だけに限定する。
3. push/pull/bothの実行結果を必要に応じてDBログへ残す。
4. Lark起点の予定をG-DXへ取り込む場合も、G-DXの予定が正本であることを壊さない。
5. `sync_source = 'lark'`の予定をアプリから再同期する条件を明確にする。
